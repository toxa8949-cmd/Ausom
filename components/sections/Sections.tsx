    ...
    <section className="home-section" style={{ padding:'80px 0', background:'var(--bg)', borderTop:'1px solid var(--border)' }}>
      <div className="w-container">
        <div style={{ marginBottom:36 }}>
          ...
          <h2 style={{ fontSize:'clamp(24px,4vw,48px)', fontWeight:800, letterSpacing:'-.025em', color:'var(--text)' }}>
        ...
        <div className="grid-4" style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:16 }}>
          {items.map(({ Icon, title, body }) => (
            ...
            onMouseEnter={e => {
              (e.currentTarget as HTMLElement).style.borderColor='var(--yellow)';
              (e.currentTarget as HTMLElement).style.boxShadow='0 4px 20px rgba(245,194,0,.12)'
            }}
            onMouseLeave={e => {
...
export function Press() {
  ...
  return (
    <section className="home-section" style={{ padding:'72px 0', background:'var(--bg-soft)', borderTop:'1px solid var(--border)', overflow:'hidden' }}>
      <div className="w-container" style={{ marginBottom:32 }}>
        <div className="s-label">Про нас пишуть</div>
        <h2 style={{ fontSize:'clamp(22px,3vw,40px)', fontWeight:800, letterSpacing:'-.02em', color:'var(--text)' }}>
      ...
      <div className="w-container" style={{ textAlign:'center', maxWidth:560, margin:'0 auto', position:'relative', zIndex:1 }}>
        ...
        {subscribed ? (
          ...
          <form style={{ display:'flex', gap:10, justifyContent:'center', flexWrap:'wrap' }} onSubmit={e=>{e.preventDefault();setSubscribed(true)}}>
            <input
              type="email" required placeholder="твій@email.com" aria-label="Email для розсилки"
              style={{ flex:'1 1 240px', minWidth:0, padding:'14px 20px', background:'rgba(255,255,255,.08)', border:'1.5px solid rgba(255,255,255,.15)', borderRadius:8, fontSize:14, color:'#fff', outline:'none', transition:'border-color .15s', fontFamily:'Inter, sans-serif' }}
