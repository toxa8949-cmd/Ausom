...
export default function Hero() {
  ...
  return (
    <section className="relative min-h-[calc(100vh-104px)] bg-[var(--bg)] overflow-hidden flex flex-col">

      {/* Subtle background accent — light: warm tint, dark: glow */}
      <div className="absolute inset-0 pointer-events-none">
        <div className="absolute top-0 right-0 w-[60%] h-full bg-gradient-to-l from-[var(--brand)]/5 to-transparent" />
        <div className="absolute bottom-0 left-0 w-full h-32 bg-gradient-to-t from-[var(--bg)] to-transparent" />
      </div>

      {/* Content */}
      <div className="container-wide flex-1 flex items-center py-16 relative z-10">
        <div className={`max-w-xl transition-all duration-200 ${anim ? 'opacity-0 -translate-x-3' : 'opacity-100 translate-x-0'}`}>

          {/* Badge */}
          <div className="inline-flex items-center gap-2 bg-[var(--brand)]/15 border border-[var(--brand)]/30 text-[var(--brand-dk)] text-[11px] font-bold tracking-[.1em] uppercase px-4 py-2 rounded-full mb-6">
            <span className="w-1.5 h-1.5 rounded-full bg-[var(--brand)] animate-pulse" />
            {s.eyebrow}
          </div>

          {/* Title */}
          <h1 className="font-display leading-none tracking-wide mb-5" style={{ fontSize: 'clamp(60px,8vw,100px)' }}>
            <span className="block text-[var(--text)]">{s.title}</span>
            <span className="block text-[var(--brand-dk)]">{s.accent}</span>
          </h1>

          <p className="text-[var(--text-2)] text-[17px] leading-relaxed mb-8 max-w-md">{s.desc}</p>

          {/* Specs */}
          <div className="flex items-center gap-6 mb-8">
            {s.specs.map(([val, label], i) => (
              <div key={i} className="flex flex-col gap-0.5">
                <span className="font-display text-[26px] text-[var(--text)] leading-none">{val}</span>
          ...
          <div className="flex items-center gap-3 mb-8 flex-wrap">
            ...
            <span className="font-display text-[36px] text-[var(--text)] tracking-wide leading-none">
              ₴{s.price.toLocaleString('uk-UA')}
            </span>
            <span className="text-[17px] text-[var(--text-3)] line-through">
        ...
        {/* Right — scooter SVG */}
        <div className={`hidden lg:flex absolute right-0 top-0 bottom-0 w-[46%] items-center justify-center transition-all duration-200 ${anim ? 'opacity-0 translate-x-4' : 'opacity-100 translate-x-0'}`}>
          <div className="relative w-full max-w-[520px] aspect-square">
            <div className="absolute inset-8 rounded-full border border-[var(--brand)]/10" />
            <div className="absolute inset-16 rounded-full border border-[var(--brand)]/6" />
            <div className="absolute inset-0 flex items-center justify-center">
              <svg viewBox="0 0 340 300" fill="none" className="w-[78%] drop-shadow-lg hover:scale-[1.02] transition-transform duration-700">
                <circle cx="68"  cy="248" r="46" stroke="var(--brand)"   strokeWidth="8"/>
                <circle cx="68"  cy="248" r="28" stroke="var(--brand)"   strokeWidth="2.5" strokeDasharray="4 6" opacity=".4"/>
                <circle cx="272" cy="248" r="46" stroke="var(--brand)"   strokeWidth="8"/>
                <circle cx="272" cy="248" r="28" stroke="var(--brand)"   strokeWidth="2.5" strokeDasharray="4 6" opacity=".4"/>
                <path d="M68 248 L108 108 L216 88 L272 248" stroke="var(--text)" strokeWidth="12" strokeLinecap="round" strokeLinejoin="round" opacity=".9"/>
                <path d="M108 108 L148 40" stroke="var(--brand)"  strokeWidth="9" strokeLinecap="round"/>
                <rect x="134" y="24" width="54" height="30" rx="8" fill="var(--brand)"/>
            ...
            {/* Floating price */}
            <div className="absolute bottom-[12%] left-[-5%] bg-[var(--bg)] border border-[var(--border)] rounded-xl px-4 py-3 shadow-lg" style={{ animation: 'floatBob 4s ease-in-out infinite' }}>
      ...
      <div className="relative z-10 grid grid-cols-2 lg:grid-cols-4 border-t border-[var(--border)]">
        {[['50K+','Задоволених клієнтів'],['#1','Бренд для дорослих'],['2Y','Офіційна гарантія'],['4.9★','Середній рейтинг']].map(([val,label],i) => (
          <div key={i} className={`flex flex-col items-center gap-1 py-5 bg-[var(--bg-mid)] ${i < 3 ? 'border-r border-[var(--border)]' : ''}`}>
