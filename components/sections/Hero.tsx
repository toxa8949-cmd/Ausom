...
// Fallback slides — used only if the `banners` table is empty OR unreachable.
// In normal operation Hero reads from Supabase and these are never rendered.
// Kept in sync with supabase/seed-ausom-banners.sql so the result matches.
...
export default function Hero() {
  ...
  // Autoplay — resets on slides/index change
  useEffect(() => {
  ...
  // Clamp index if slides shrink (e.g. admin disables one in another tab)
  useEffect(() => {
  ...
  return (
    <section style={{ background:'var(--bg)', borderBottom:'1px solid var(--border)' }}>
      ...
      {/* Product strip — only when this slide is linked to a product */}
      {product && (
