...
import { Save, Plus, X, ArrowLeft, Upload, Trash2, GripVertical } from 'lucide-react'
...
export default function ProductForm({ params }: { params: Promise<{ id: string }> }) {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    } catch (e: any) { se(e.message || 'Помилка збереження') }
    finally { sv(false) }
  }
  ...
  const I = ({ k, lbl: label, p, type='text', req }: { k: keyof F; lbl: string; p: string; type?: string; req?: boolean }) => (
    <div>
      ...
      <input type={type} value={String(form[k])} onChange={e => set(k, e.target.value)} required={req}
          ...
          <div>
              ...
              <div style={{ display:'flex',flexDirection:'column',gap:14 }}>
                ...
                <div>
                  ...
                  <textarea value={form.description} onChange={e => set('description', e.target.value)} rows={4}
                    ...
                    onFocus={e => (e.target.style.borderColor = '#F5C200')}
                    onBlur={e => (e.target.style.borderColor = '#EEEEEE')} placeholder="Опис товару..." />
                </div>
              </div>
            </Card>
            ...
            <Card title="Зображення">
              <div style={{ display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:10,marginBottom:14 }}>
                {form.images.map((url, i) => (
                  <div key={i} style={{ position:'relative',aspectRatio:'1',borderRadius:8,overflow:'hidden',border:B,background:'#F9F9F9' }}>
                    ...
                    <button type="button" onClick={() => removeImage(i)}
                      style={{ position:'absolute',top:4,right:4,width:24,height:24,borderRadius:6,background:'rgba(0,0,0,.7)',border:'none',color:'#fff',cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center' }}>
                      <X size={12} />
                    </button>
                ...
                <label style={{ aspectRatio:'1',borderRadius:8,border:'2px dashed #DDD',display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',cursor: uploading ? 'wait' : 'pointer',background:'#FAFAFA',transition:'border-color .15s' }}>
                  ...
                  {uploading ? (
                    ...
                    <>
                      ...
                      <span style={{ fontSize:10,color:'#BBB',marginTop:6,fontWeight:600 }}>Додати</span>
                    </>
                  )}
                </label>
              </div>
              ...
              <div style={{ display:'flex',flexDirection:'column',gap:8 }}>
                {form.features.map((feat, i) => (
                  <div key={i} style={{ display:'flex',gap:8 }}>
                    <input value={feat} onChange={e => { const a = [...form.features]; a[i] = e.target.value; set('features', a) }}
                      style={{ ...inp, flex:1 }} placeholder={`Особливість ${i + 1}`}
                      onFocus={e => (e.target.style.borderColor = '#F5C200')}
                      onBlur={e => (e.target.style.borderColor = '#EEEEEE')} />
                    {form.features.length > 1 && (
                      <button type="button" onClick={() => set('features', form.features.filter((_, j) => j !== i))}
                        style={{ width:38,height:38,flexShrink:0,display:'flex',alignItems:'center',justifyContent:'center',background:'#FEF2F2',border:'1.5px solid #FECACA',borderRadius:7,cursor:'pointer',color:'#EF4444' }}>
                        <X size={14} />
                      </button>
                    )}
                  </div>
                ...
                <button type="button" onClick={() => set('features', [...form.features, ''])}
                  style={{ display:'inline-flex',alignItems:'center',gap:6,fontSize:12,fontWeight:600,color:'#666',background:'#F9F9F9',border:B,padding:'8px 14px',borderRadius:7,cursor:'pointer',width:'fit-content' }}>
                  <Plus size={12} /> Додати
                </button>
              </div>
            </Card>
              ...
              <div style={{ display:'flex',flexDirection:'column',gap:14 }}>
                <div>
                  ...
                  <select value={form.category} onChange={e => set('category', e.target.value)} style={{ ...inp, cursor:'pointer' }}>
                    <option value="commuter">Міський</option><option value="offroad">Позашляховий</option>
                  </select>
                </div>
                <div>
                ...
                {([['in_stock','В наявності'],['is_new','Новинка'],['is_featured','На головній']] as [keyof F, string][]).map(([k, la]) => (
                  <label key={String(k)} style={{ display:'flex',alignItems:'center',justifyContent:'space-between',cursor:'pointer' }}>
                    ...
                    <button type="button" onClick={() => set(k, !form[k])}
                      style={{ width:44,height:24,borderRadius:12,border:'none',cursor:'pointer',background:(form[k] as boolean)?'#F5C200':'#EEE',transition:'background .2s',position:'relative' }}>
