...
export default function CatalogBrandContent({ brand, cfg }: Props) {
  ...
  const headerStyle = { background: 'var(--bg-soft)', borderBottom: '1px solid var(--border)', padding: '40px 0 32px' }
      const breadStyle = { display: 'flex', alignItems: 'center', gap: 8, fontSize: 13, color: 'var(--text-3)', marginBottom: 16 }
      const linkStyle = { color: 'var(--text-3)', textDecoration: 'none' as const }
      const h1Style = { fontSize: 'clamp(28px,4vw,52px)', fontWeight: 800, letterSpacing: '-.03em', color: 'var(--text)', lineHeight: 1.05 }
      const introStyle = { fontSize: 14, color: 'var(--text-3)', marginTop: 12, maxWidth: 640 }
      const countStyle = { fontSize: 13, color: 'var(--text-3)', marginTop: 8 }
                    ...
                    <div style={headerStyle}>
                                <div className="w-container">
                                          <div style={breadStyle}>
                                                      <Link href="/" style={linkStyle}>Головна</Link>Link>
                                                      <ChevronRight size={13} />
                                                      <Link href="/catalog" style={linkStyle}>Каталог</Link>Link>
                                                      <ChevronRight size={13} />
                                                      <span style={{ color: 'var(--text)', fontWeight: 500 }}>{cfg.nameUA}</span>span>
                                          </div>div>
                                          <div className="s-label">Каталог 2026</div>div>
                                          <h1 style={h1Style}>{cfg.h1}</h1>h1>
                                          <p style={introStyle}>{cfg.intro}</p>p>
                                          <p style={countStyle}>{loading ? 'Завантаження...' : products.length + ' моделей'}</p>p>
                                </div>div>
                    </div>div>
                ...
                <div className="w-container" style={{ paddingTop: 32, paddingBottom: 64 }}>
                    ...
                    {!loading && products.length === 0 && (
                        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '80px 0', textAlign: 'center', gap: 16 }}>
                                    <span style={{ fontSize: 48, opacity: 0.3 }}>&#x1F6F4;</span>span>
