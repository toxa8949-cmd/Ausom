          ...
          {post.cover_image ? (
                  <div style={{ position:'relative', height:360, overflow:'hidden' }}>
                              ...
                              <div className="w-container" style={{ position:'relative', zIndex:1, height:'100%', display:'flex', flexDirection:'column', justifyContent:'flex-end', paddingBottom:40 }}>
                                            <span style={{ fontSize:11, fontWeight:700, letterSpacing:'.1em', textTransform:'uppercase' as const, color:'#F5C200', background:'rgba(245,194,0,.15)', border:'1px solid rgba(245,194,0,.3)', padding:'4px 12px', borderRadius:4, display:'inline-block', marginBottom:14, width:'fit-content' }}>{post.category}</span>
                  ...
                  <div style={{ background:'var(--bg-soft)', borderBottom:'1px solid var(--border)', padding:'48px 0 40px' }}>
                            <div className="w-container">
                                        ...
                                        <span style={{ fontSize:11, fontWeight:700, letterSpacing:'.1em', textTransform:'uppercase' as const, color:'#8B6800', background:'#FFF3CC', border:'1px solid #F5C200', padding:'4px 12px', borderRadius:4, display:'inline-block', marginBottom:20 }}>{post.category}</span>
                                        <h1 style={{ fontSize:'clamp(28px,4vw,52px)', fontWeight:800, letterSpacing:'-.03em', color:'var(--text)', lineHeight:1.1, maxWidth:800, marginBottom:16 }}>{post.title}</h1>
              ...
              <div className="w-container" style={{ padding:'48px 40px 80px' }}>
                      ...
                      <div style={{ marginBottom:40, padding:24, background:'var(--bg-soft)', borderRadius:12, border:'1.5px solid var(--border)' }}>
                                    <p style={{ fontSize:18, fontWeight:500, color:'var(--text-2)', lineHeight:1.7 }}>{post.excerpt}</p>
                      </div>
                                )}
