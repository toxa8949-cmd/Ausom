  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <div className="border-b border-[#e8e8e5] py-4">
        <div className="container-wide flex items-center gap-2 text-sm text-[#888884]">
          <Link href="/" className="hover:text-[#0b0b0b] transition-colors">Головна</Link>
          <span>/</span>
          <Link href="/catalog" className="hover:text-[#0b0b0b] transition-colors">Каталог</Link>
          <span>/</span>
      ...
      <div className="container-wide py-12">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16">
          ...
          <div className="bg-[#f4f4f2] rounded-2xl aspect-square flex items-center justify-center relative">
            ...
            {product.tag && (
              <span className="absolute top-5 right-5 bg-[#0b0b0b] text-white text-xs font-bold uppercase tracking-wider px-3 py-1.5 rounded-lg">
                {product.tag}
              </span>
            )}
            <svg viewBox="0 0 300 260" fill="none" className="w-3/4 h-3/4">
              <circle cx="60" cy="208" r="42" stroke="#ff5c00" strokeWidth="8"/>
              <circle cx="240" cy="208" r="42" stroke="#ff5c00" strokeWidth="8"/>
              <path d="M60 208 L96 92 L188 72 L240 208" stroke="#1a1a1a" strokeWidth="12" strokeLinecap="round" strokeLinejoin="round"/>
              <path d="M96 92 L132 34" stroke="#ff5c00" strokeWidth="9" strokeLinecap="round"/>
              <rect x="118" y="20" width="48" height="26" rx="7" fill="#ff5c00"/>
          ...
          <div>
            ...
            <div className="flex gap-2 mb-4 flex-wrap">
              <span className="bg-[#f4f4f2] text-[#888884] text-xs font-semibold px-3 py-1 rounded-full uppercase tracking-wider">
              ...
              {product.motor === 'dual' && (
                <span className="bg-[#ff5c00]/10 text-[#ff5c00] text-xs font-semibold px-3 py-1 rounded-full uppercase tracking-wider">
                  Подвійний мотор
                </span>
              )}
              {product.is_new && (
                <span className="bg-[#0b0b0b] text-white text-xs font-bold px-3 py-1 rounded-full uppercase tracking-wider">
                  Новинка
                </span>
              )}
            </div>

            <h1 className="text-3xl font-bold text-[#0b0b0b] mb-4 leading-tight">{product.name}</h1>
            ...
            <div className="grid grid-cols-2 gap-3 mb-8">
              {[
                ...
                return (
                  <div key={spec.label} className="bg-[#f4f4f2] rounded-xl p-4 flex items-center gap-3">
                    <Icon size={18} className="text-[#ff5c00] shrink-0" />
                    <div>
                      <div className="text-[11px] text-[#888884] font-medium uppercase tracking-wider">{spec.label}</div>
                      <div className="text-sm font-bold text-[#0b0b0b]">{spec.value}</div>
                    </div>
                  </div>
                )
              })}
            ...
            <div className="flex gap-4 flex-wrap border-t border-[#e8e8e5] pt-6">
              {['🚚 Безкоштовна доставка', '🛡 Гарантія 2 роки', '🔄 14 днів повернення'].map(b => (
                <span key={b} className="text-xs text-[#888884] font-medium">{b}</span>
              ))}
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 mt-20 pt-16 border-t border-[#e8e8e5]">
          <div>
            ...
            <p className="text-[#888884] leading-relaxed">{product.description}</p>
          </div>
          <div>
            ...
            <ul className="space-y-3">
              {product.features.map(f => (
                <li key={f} className="flex items-center gap-3 text-sm">
                  <span className="w-5 h-5 bg-[#ff5c00] rounded-full flex items-center justify-center shrink-0">
